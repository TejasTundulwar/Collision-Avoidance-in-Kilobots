...
void message_rx(message_t *m, distance_measurement_t *d)
{
    ...
    #ifdef SIMULATOR
    printf("distance between %d and %d is %d\n",mydata->my_id,m->data[ID],dist);
    #endif
    
    if(dist > even_close)
    {//stop or continue
    if(dist < min_dist && (mydata->my_id < m->data[ID])) //&& (m->data[MOTION_STATE] != DISABLED) )
    {
        //If distance less than minimum set value bot stops
        mydata->motion_state=DISABLED;
        
        #ifdef SIMULATOR
        printf("distance between %d and %d is less so STOPPED\n",mydata->my_id,m->data[ID]);
        #endif
    }
    else {
    ...
    if (dist <= even_close && (mydata->my_id < m->data[ID])){
        mydata->motion_state=ACTIVE;
    }
    else if (dist <= even_close && (mydata->my_id > m->data[ID])){
        mydata->motion_state=DISABLED;
    }
    ...
    if (m->type == NORMAL)
    {
        recv_sharing(m->data, dist);//stores distance of neighbours
...
void loop()
{
    ...
    //if(mydata->now % 100 <= 50) {
    	// mydata->motion_state = DISABLED;
	/*#ifdef SIMULATOR
	   printf("MOTION STATE = DISABLED :: %d\n", (mydata->now % 40));
        #endif*/
   // } else {
	// mydata->motion_state = ACTIVE;
    //}
